...
def dashboard(request):
    ...
    if budget and budget.limit:
      ...
      if percent_used >= 100:
        messages.error(request, f"🚨 You have exceeded your budget for {current_month_name}!")
    elif percent_used >= 80:
        messages.warning(request, f"⚠️ You’ve spent {percent_used:.1f}% of your budget for {current_month_name}.")
...
def export_csv(request):
    ...
    income_data = list(incomes.values('date', 'source', 'amount'))
    expense_data = list(expenses.values('date', 'category', 'amount'))

    df_income = pd.DataFrame(income_data)
    df_expense = pd.DataFrame(expense_data)

    buffer = io.StringIO()
    buffer.write(f"Income for {month}\n")
    df_income.to_csv(buffer, index=False)
    buffer.write(f"\nExpenses for {month}\n")
    df_expense.to_csv(buffer, index=False)

    response = HttpResponse(buffer.getvalue(), content_type='text/csv')
    response['Content-Disposition'] = f'attachment; filename="{month}_report.csv"'
    return response
...
def export_pdf(request):
    ...
    template = get_template('finance/pdf_template.html')
    ...
    response['Content-Disposition'] = f'attachment; filename="{month}_report.pdf"'
